...
Lemma mylemma_map_snoc : forall (X Y : Type) (f : X -> Y) (x : X) (l : list X), map f (snoc l x) = snoc (map f l) (f x).
Proof.
  intros.
...
Proof.
  ...
  intros.
  unfold override.
  destruct (beq_nat k1 k2) eqn:beq12.
  - apply beq_nat_true in beq12.
    rewrite <- beq12.
    rewrite -> H.
    reflexivity.
  - reflexivity.
